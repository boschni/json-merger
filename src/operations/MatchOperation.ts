    ...
    processInArray(
        ...
        // Handle $match.path
            ...
            if (JsonPointer.get(resultArray, keywordValue.path) !== undefined) {
                const decodedPath = JsonPointer.decode(keywordValue.path)[0];

                matchedResultArrayIndex = parseInt(decodedPath.toString());
            }
        }
        ...
        if (matchedResultArrayIndex <= resultArrayIndex) {
            resultArrayIndex +=
                result.resultArrayIndex - matchedResultArrayIndex;
        }
